class Node {
	...
	private function hanger() {
		var m:MovieClip;
		for (var i:Number = 0; i<=_root.maxhang; i++) {
			m = eval("_root.hanger"+i);
			if (mymc.hitTest(m.zone) && !_root.hangfull[i]) {
				mymc._x = m._x;
				mymc._y = m._y;
				_root.hangfull[i] = true;
				hookedon = i;
				vx = 0;
				vy = 0;
				hooked = true;
